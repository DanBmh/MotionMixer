...
def train(model, model_name, args):
    ...
    for epoch in range(args.n_epochs):
        ...
        for cnt, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            ].reshape(-1, args.output_n, args.pose_dim)
...
def test_mpjpe(model, args):
    ...
    Dataset = Datasets(
        args.data_dir, args.input_n, args.output_n, args.skip_rate, split=2
    )
    ...
    with torch.no_grad():
        for cnt, batch in enumerate(loader_test):
            ...
            sequences_train = batch[:, 0 : args.input_n, joint_used, :].view(
                -1, args.input_n, args.pose_dim
            )

            sequences_predict_gt = batch[
                :, args.input_n : args.input_n + args.output_n, full_joint_used, :
            ]  # .view(-1,args.output_n,args.pose_dim)

            sequences_predict = model(sequences_train).view(
                -1, args.output_n, 18, 3
            )  # .permute(0,1,3,2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", default=200, type=int, required=False
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="./checkpoints/amass_3d_25frames_ckpt",
    ...
    parser_mpjpe.add_argument(
        "--channels_mlp_dim", default=128, type=int, required=False
    )
    parser_mpjpe.add_argument(
