    ...
    for i in np.arange(len(I)):
        ...
        if not update:
            ...
            if i == 0:
                ...
                plots.append(
                    ax.plot(
                        x, y, z, lw=2, linestyle="--", c=lcolor if LR[i] else rcolor
                    )
                )
...
def visualize(
    ...
    for action in actions:
        ...
        for cnt, batch in enumerate(loader):
            ...
            sequences_train = batch[:, 0:input_n, dim_used].view(
                -1, input_n, len(dim_used)
            )
            sequences_gt = batch[:, input_n : input_n + output_n, :]

            sequences_predict = (
                modello(sequences_train).contiguous().view(-1, output_n, len(dim_used))
            )
            ...
            data_pred = (
                torch.squeeze(all_joints_seq, 0).cpu().data.numpy() / 1000
            )  # in meters
            ...
            line_anim.save(
                "./visualizations/pred{}/human_viz{}.gif".format(25, i), writer="pillow"
            )
