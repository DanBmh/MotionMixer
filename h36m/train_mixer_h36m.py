...
def train(model, model_name, args):
    ...
    if args.loss_type == "mpjpe":
        ...
        dataset = H36M_Dataset_Angle(
            args.data_dir, args.input_n, args.output_n, args.skip_rate, split=0
        )
        vald_dataset = H36M_Dataset_Angle(
            args.data_dir, args.input_n, args.output_n, args.skip_rate, split=1
        )
        dim_used = np.array(
            [
        ...
        for cnt, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            if args.delta_x:
                ...
                loss = torch.mean(
                    torch.sum(
                        torch.abs(
                            sequences_predict.reshape(-1, args.output_n, len(dim_used))
                            - sequences_gt
                        ),
                        dim=2,
                    ).view(-1)
...
def test_mpjpe(model, args):
    ...
    for action in actions:
        ...
        if args.loss_type == "mpjpe":
            ...
            dataset_test = H36M_Dataset_Angle(
                args.data_dir,
                args.input_n,
                args.output_n,
                args.skip_rate,
